...
| LI.FI | `LIFI_INTEGRATOR`（可選 `LIFI_API_KEY`） | 無 Key 可用公用 API；Key 只提高額度 |
| Logo.dev | `LOGO_DEV_TOKEN` | 可選；預設 logo 為免費公用（見 [API_KEYS.zh-TW.md](API_KEYS.zh-TW.md)） |
...
可選：`LIFI_API_KEY`、`LOGO_DEV_PUBLISHABLE_KEY` → `LOGO_DEV_TOKEN`（預設 deploy 不注入）。
