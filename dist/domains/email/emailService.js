...
© 2026 ${appName}. All rights reserved.
      `.trim();
            (0, logger_1.logDebug)('Sending verification email', { email });
            ...
            (0, logger_1.logBusinessEvent)('verification_email_sent', undefined, { email, messageId: response.data?.id || 'unknown' });
            return true;
        }
        catch (error) {
            (0, logger_1.logError)('Failed to send verification email', error, { email });
            return false;
        }
    }
    /**
...
© 2026 ${appName}. All rights reserved.
      `.trim();
            (0, logger_1.logDebug)('Sending password reset email', { email });
            const response = await resend.emails.send({
                from: fromEmail,
                to: email,
            ...
            (0, logger_1.logBusinessEvent)('password_reset_email_sent', undefined, { email, messageId: response.data?.id || 'unknown' });
            return true;
        }
        catch (error) {
