...
exports.PlaidAuthService = void 0;
...
class PlaidAuthService {
    ...
    static async createLinkToken(userId) {
        ...
        if (!plaidRedirectUri) {
            ...
            const webAppOrigin = allowedOrigins.find(origin => origin.includes('localhost') && (origin.includes(':3000') || origin.includes(':3001')));
            if (webAppOrigin) {
        ...
        try {
            ...
            if (errorCode === 'INVALID_FIELD' && errorMessage?.includes('country')) {
                throw new Error(`Plaid does not support the selected country codes (${supportedCountryCodes.join(', ')}). Please verify your Plaid account has these countries enabled or update PLAID_COUNTRY_CODES.`);
            }
            // 其他 INVALID_FIELD 錯誤
            if (errorCode === 'INVALID_FIELD') {
                throw new Error(`Plaid API returned an INVALID_FIELD error: ${displayMessage || errorMessage || 'Unknown error'}. Please check PLAID_REDIRECT_URI and related configuration.`);
            }
            // Plaid API 連線錯誤
            if (errorCode === 'INVALID_REQUEST') {
                throw new Error(`Plaid API returned INVALID_REQUEST: ${displayMessage || errorMessage || 'Please check API credentials and configuration'}.`);
            }
            // 通用錯誤
            throw error;
        }
    ...
    static decryptPlaidItem(item) {
        const decryptedAccessToken = encryption_1.EncryptionUtil.decrypt(item.accessToken);
        const decryptedItemId = encryption_1.EncryptionUtil.decrypt(item.itemId);
        return {
...
exports.PlaidAuthService = PlaidAuthService;
//# sourceMappingURL=plaidAuthService.js.map