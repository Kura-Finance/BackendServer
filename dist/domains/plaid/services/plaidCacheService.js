    ...
    static async saveFinanceSnapshotToCache(userId, snapshot, pendingCursorUpdates = []) {
        ...
        try {
            ...
            await prisma_1.prisma.$transaction(async (tx) => {
                ...
                const hasValidItemRef = (pid) => pid == null || validItemIds.has(pid);
                ...
                const droppedCount = (snapshot.accounts.length - accountsForCache.length) +
                ...
                if (droppedCount > 0) {
                    logger_1.appLogger.warn('Dropped Plaid cache rows referencing removed items during sync', {
                        userId,
                        droppedCount,
                    });
                }
                ...
                if (accountsForCache.length > 0) {
                    const accountsToCache = accountsForCache.map((acc) => {
                        const split = (0, plaidPayloadBuilder_1.splitAccount)(acc, acc.plaidItemId ?? null, 'banking');
                        return {
                ...
                if (transactionsForCache.length > 0) {
                    const transactionsToCache = transactionsForCache.map((tx2) => {
                        const split = (0, plaidPayloadBuilder_1.splitTransaction)(tx2, tx2.plaidItemId ?? null);
                        return {
                            accountId: split.metadata.accountId,
                ...
                if (investmentAccountsForCache.length > 0) {
                    const investmentAccountsToCache = investmentAccountsForCache.map((acc) => {
                        const split = (0, plaidPayloadBuilder_1.splitInvestmentAccount)(acc, acc.plaidItemId ?? null);
                        return {
                ...
                if (investmentsForCache.length > 0) {
                    const investmentsToCache = investmentsForCache.map((inv) => {
                        const split = (0, plaidPayloadBuilder_1.splitInvestment)(inv, inv.plaidItemId ?? null);
                        return {
                            plaidItemId: split.metadata.plaidItemId,
                            accountId: split.metadata.accountId,
            ...
            try {
                ...
                logger_1.appLogger.warn('Failed to GC orphaned payload keys (cache already saved)', {
                    userId,
